...
// --- Config WiFi & MQTT ---
const char ssid[] = "pakkapon2006"; 
const char pass[] = "33333333";
...
void loop() {
  ...
  // --- เช็คเส้นตัดขวาง 5 จุด (จุดเปลี่ยนสถานะสำคัญ) ---
  if (v1 == LINE && v2 == LINE && v3 == LINE && v4 == LINE && v5 == LINE) {
      
      // CASE: ถึงห้องเป้าหมาย (ขาไป)
      if (isDelivering) {
          stopCar(); 
          lcd.setCursor(0, 1); lcd.print("Arrived Room   ");
          while (getLoadDistance() < LOAD_THRESHOLD) {
             stopCar();
             delay(100);
          ...
          delay(1000); 
          spinBack170(); // หมุนกลับเพื่อเริ่มขากลับ
          isDelivering = false;
          isReturning = true;
          readyToCount = true;
          intersectionCount = 0; 
      ...
      // CASE: ถึงจุดเริ่มต้น (ขากลับ)
          ...
          if (arrivedHome) {
              ...
              delay(500);
              spinBack170();   // หมุน 170 องศาตามโจทย์
              stopCar();       // หยุดสนิทรอคำสั่งถัดไป
              isReturning = false;
              targetRoom = 0;
...
// ฟังก์ชันหมุนกลับตัว 170 องศา
void spinBack170() { 
    stopCar(); delay(200);
    motorControl(SPD_SPIN, SPD_SPIN, false, false); delay(300); // ถอยหลังตั้งหลักเล็กน้อย
    stopCar(); delay(200);
    
    // หมุนตัว
    motorControl(SPD_SPIN, SPD_SPIN, true, false); 
    delay(550); // ปรับค่านี้เพื่อความแม่นยำของมุม 170 องศา
    
    unsigned long startSearch = millis();
    while(digitalRead(S3) != LINE) {
        if (millis() - startSearch > 3000) break; // Time out ป้องกันหมุนไม่หยุด
    }
    stopCar(); delay(200);
}
